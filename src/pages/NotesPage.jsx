...
const NotesPage = () => {
  ...
  // Filter notes by search query and active course
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 animate-fade-in">
      ...
      <div className="space-y-6">
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-100 dark:border-slate-700/80 shadow-sm space-y-4">
          ...
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-widest pl-1 block mb-2">My Courses</span>
            <button
      ...
      {/* Main Content Area: Notes List, Editor or Details */}
            ...
            <form onSubmit={handleCreateOrUpdateNote} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2.5 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none dark:text-white"
                    required
                  >
                    {courses.map((c) => (
                      <option key={c._id} value={c._id}>{c.code} - {c.title}</option>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none dark:text-white font-bold"
                  required
                />
              </div>
              <div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-100 dark:border-slate-700/80 shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-3">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => openEditNote(selectedNote)} className="text-slate-400 hover:text-indigo-600 flex items-center justify-center p-1.5 hover:bg-indigo-50 rounded-lg">
                    <Icon name="edit" className="text-lg" />
                  </button>
                  <button onClick={() => handleDeleteNote(selectedNote._id)} className="text-slate-400 hover:text-rose-600 flex items-center justify-center p-1.5 hover:bg-rose-50 rounded-lg">
                    <Icon name="delete" className="text-lg" />
                  </button>
                  <button onClick={() => handleSummarizeNote(selectedNote._id)} className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 text-white text-xs font-bold flex items-center gap-1 transition-all shadow-md">
                    <Icon name="auto_awesome" fill={1} className="text-sm" /> Summarize
                  </button>
                </div>
              </div>
              
              <div className="space-y-2">
                <h1 className="text-2xl font-black text-slate-800 dark:text-white">{selectedNote.title}</h1>
                <div className="flex items-center gap-3">
              ...
              {/* Note Content */}
              <div className="text-slate-700 dark:text-slate-200 text-sm leading-relaxed whitespace-pre-line leading-relaxed font-medium bg-slate-50/50 dark:bg-slate-900/30 p-5 rounded-2xl border border-slate-100 dark:border-slate-800">
              ...
              {selectedNote.aiSummary && (
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/20 dark:to-purple-950/20 p-5 rounded-2xl border border-indigo-100/60 dark:border-indigo-900/30 space-y-3.5">
                  ...
                  <p className="text-xs font-semibold text-slate-600 dark:text-slate-300 leading-relaxed leading-relaxed">{selectedNote.aiSummary}</p>
                    ...
                    <div className="space-y-2.5 pt-2">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {selectedNote.keyInsights.map((insight, idx) => (
                          <div key={idx} className="p-3 bg-white dark:bg-slate-900/65 rounded-xl border border-indigo-50 dark:border-indigo-900/30">
                            <h4 className="font-bold text-xs text-indigo-950 dark:text-indigo-300">{insight.title}</h4>
                            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">{insight.description}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  )}
                  {selectedNote.transcriptionQuality && (
                    <div className="flex items-center gap-1.5 pt-1.5 text-xs font-semibold text-slate-400">
                      <Icon name="verified" className="text-indigo-500" /> Audio transcription index score: {selectedNote.transcriptionQuality}%
                    </div>
                  )}
                </div>
              )}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/80 shadow-sm flex flex-col h-[calc(100vh-8rem)] xl:h-auto min-h-[500px]">
        <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 dark:text-slate-400 mt-0.5">
              {selectedNote ? `Discussing: ${selectedNote.title}` : 'General campus assistant'}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div
                  className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-xs font-medium leading-relaxed ${
                    msg.role === 'user'
                      ...
                      : 'bg-slate-100 dark:bg-slate-900 dark:text-white rounded-tl-none border border-slate-50 dark:border-slate-800'
                  }`}
                >
                  {msg.content}
                </div>
          ...
          {sendingChat && (
            <div className="flex justify-start">
              <div className="bg-slate-100 dark:bg-slate-900 rounded-2xl rounded-tl-none p-3.5 border border-slate-50 dark:border-slate-800">
                <Loader size="sm" />
              </div>
            </div>
          )}
        ...
        <form onSubmit={handleSendChat} className="p-3 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/10 rounded-b-2xl flex gap-2">
          <input
            ...
            className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs focus:outline-none dark:text-white"
          />
          <button
            type="submit"
